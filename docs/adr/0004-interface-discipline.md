...
- **Uniform output policy.** `reset()` clears the furigana output (DOM element and onChange) through the same output policy as every other transition, so the framework state never desyncs from the DOM.
